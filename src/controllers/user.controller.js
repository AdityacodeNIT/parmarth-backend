...
// this is the function for generating access tokenwhere we caan use these
// token by exporting them from because they are quite common
const generateAccessAndRefreshtoken = async (userId) => {
  try {
    ...
    if (!save) {
      throw new ApiError(402, "User could not be saved");
    }
...
const registerUser = asyncHandler(async (req, res) => {
  ...
  return res
    .status(201)
    .json(new ApiResponse(200, createdUser, "Aalu registered succesfully"));
});
...
const getUserChnanelProfile = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, channel[0], "User channel fethced succesfully"));
});
export {
  registerUser,
  loginUser,
